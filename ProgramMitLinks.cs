...
namespace SAwareness
{
    ...
    static class Menu
    {
        ...
        public static MenuItemSettings InhibitorHealth = new MenuItemSettings(); //Missing everything
        ...
        public static MenuItemSettings Range = new MenuItemSettings(typeof(SAwareness.Ranges)); //Maybe every range bugged by lol
    ...
    {
        //http://botoflegends.com/forum/topic/5155-script-waiteee-aka-wait-for-it-countdowns-for-ga-zhonya-immunes/page-2
    ...
    class DestinationTracker
    {
        /*function FindNearestNonWall(x0, y0, z0, maxRadius, precision) --returns the nearest non-wall-position of the given position(Credits to gReY)
            //https://bitbucket.org/ViceV/where-did-he-go/src/b1484bbb5ea9b0d5359d3f41c33c75a8fd1b09aa/WhereDidHeGo.lua?at=master
        end*/
    ...
    class AutoLevler
    {
        ...
        private void LoadLevelFile()
        {
            ...
            loc = loc + "\\Config\\SCooldownGUI\\autolevel.conf";
            if (!File.Exists(loc))
            {
    ...
    class CdTracker
    {
        ...
        */
        ...
        {
            if (!Menu.ItemPanel.GetActive())
                return;
            var loc = Assembly.GetExecutingAssembly().Location;
            loc = loc.Remove(loc.LastIndexOf("\\", StringComparison.Ordinal));
            loc = loc + "\\Sprites\\SCooldownGUI\\";

            foreach (var enemy in Enemies)
            {
                var i1 = enemy.Key.InventoryItems;
                    ...
                    if (File.Exists(loc + "ITEMS\\" + inventorySlot.Id + ".dds"))
                    {
                        ...
                        for (int i = 0; i < enemy.Value.SGui.Item.Length; i++)
                        {
                            if (inventorySlot.Slot >= 0 && inventorySlot.Slot <= 6)
                            {
                                ...
                                if (champ.SGui.Item[i].Texture == null || champ.SGui.ItemId[i] != inventorySlot.Id)
                                {
                                    champ.SGui.ItemId[i] = inventorySlot.Id;
                ...
                {
                    int id = unusedId[i];
                    if (id == i && champ.SGui.Item[i].Texture != _overlayEmptyItem)
                    {
                        champ.SGui.Item[i].Texture = _overlayEmptyItem;
                    }
                }
            }
        }
    ...
    class Program
    {
        ...
        static void CreateMenu()
        {
            ...
            try
            {
                ...
                Menu.CdPanel.MenuItems.Add(Menu.CdPanel.Menu.AddItem(new LeagueSharp.Common.MenuItem("SAwarenessItemPanel", "ItemPanel").SetValue(true)));
